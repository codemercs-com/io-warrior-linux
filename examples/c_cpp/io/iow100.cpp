/*
 * Simple IO sample to write and read IO-Ports of IO-Warrior100.
 * Only first found IO-Warrior will be used by default. If no IOW100, exit
 */
...
void drain_buffers(IOWKIT_HANDLE handle)
{
	...
	while (IowKitReadNonBlocking(handle, IOW_PIPE_IO_PINS, (char*)&report, IOWKIT100_IO_REPORT_SIZE))
	{
	}
}

// Write IO-Report to IO-Warrior100
ULONG write_io(IOWKIT_HANDLE handle, IOWKIT100_IO_REPORT report)
{
	unsigned int ret = IowKitWrite(handle, IOW_PIPE_IO_PINS, (char *)&report, IOWKIT100_IO_REPORT_SIZE);
	return ret;
}
...
ULONG read_io(IOWKIT_HANDLE handle)
{
	...
	if(IowKitRead(handle, IOW_PIPE_IO_PINS, (char *)&report, IOWKIT100_IO_REPORT_SIZE) == IOWKIT100_IO_REPORT_SIZE)
	{
		for (ULONG  j = 0; j < (IOWKIT100_IO_REPORT_SIZE-1); j++)
			printf("%02x ", report.Bytes[j]);

		printf("\n");

		return IOWKIT100_IO_REPORT_SIZE;
	}
	else
		return 0;
}
...
ULONG read_io_nonblock(IOWKIT_HANDLE handle)
{
	...
	if(IowKitReadNonBlocking(handle, IOW_PIPE_IO_PINS, (char *)&report, IOWKIT100_IO_REPORT_SIZE) == IOWKIT100_IO_REPORT_SIZE)
	{
		for (ULONG  j = 0; j < (IOWKIT100_IO_REPORT_SIZE-1); j++)
			printf("%02x ", report.Bytes[j]);
...
{
	IOWKIT100_SPECIAL_REPORT rep;
	memset(&rep, 0x00, IOWKIT100_SPECIAL_REPORT_SIZE);

	rep.ReportID = 0xFF;
	if (IowKitWrite(handle, IOW_PIPE_SPECIAL_MODE, (char *)&rep, IOWKIT100_SPECIAL_REPORT_SIZE) == IOWKIT100_SPECIAL_REPORT_SIZE)
	{
		if (IowKitRead(handle, IOW_PIPE_SPECIAL_MODE, (char *)&rep, IOWKIT100_SPECIAL_REPORT_SIZE) == IOWKIT100_SPECIAL_REPORT_SIZE)
		{
			for (ULONG j = 0; j < (IOWKIT100_IO_REPORT_SIZE-1); j++)
			...
			return IOWKIT100_SPECIAL_REPORT_SIZE;
		}
		else
			return 0;
	}
...
int main(int argc, char *argv[])
{
	...
	{
		printf("No IO-Warrior100 found on first position");
	...
	printf("PID %x (IO-Warrior100), S/N \"%s\", Rev %lX\n", (unsigned int)pid, snt, rev);
	...
	for (int i = 0; i < 20; i++)
	{
		...
		//write_io(handle, report);
		if(write_io(handle, report) != IOWKIT100_IO_REPORT_SIZE)
		{
...
/*
	...
	for (int i = 0; i < 100; i++)
	{
		if (read_io(handle) != IOWKIT100_IO_REPORT_SIZE)
...
/*
	...
	for (int i = 0; i < 100; i++)
	{
		if (read_io_nonblock(handle) != IOWKIT100_IO_REPORT_SIZE)
			printf("-- No new data\n");
...
*/
	...
	for (int i = 0; i < 100; i++)
	{
		if (read_io_specialmode(handle) != IOWKIT100_SPECIAL_REPORT_SIZE)
