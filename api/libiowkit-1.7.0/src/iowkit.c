...
static char KitVersion[] = "IO-Warrior Kit V1.7.0.21";

// A mutex to protect the open call
static pthread_mutex_t device_mutex = PTHREAD_MUTEX_INITIALIZER;

typedef struct _IowDevice_t
{
  ...
  for (i = 0; i < IOWKIT_MAX_DEVICES * 2; i++)
  {
    ...
    if (fd != -1)
    {
      ...
      if (ioctlret != -1)
      {
        ...
        //Count if last endpoint found for productID
        if((info.product == IOWKIT_PID_IOW24) ||
           (info.product == IOWKIT_PID_IOW40) ||
           (info.product == IOWKIT_PID_IOW56))
           {
              if (info.if_num == 1)
                numIoWarriors++;
           }

        if((info.product == IOWKIT_PID_IOW28) ||
           (info.product == IOWKIT_PID_IOW100)) 
           {
              if (info.if_num == 3)
                numIoWarriors++;
           }
